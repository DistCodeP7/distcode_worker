...
func (d *JobDispatcher) processJob(ctx context.Context, job types.JobRequest) {
	...
	if job.ProblemId == 1 {
		...
		go func() {
			// Pass the slice of submitted file contents directly. Filenames are autogenerated inside the runner.
			runErrCh <- RunEphemeralJob(jobCtx, d.dockerCli, d.workerImage, "centralized_mutex", job.Code, stdoutCh, stderrCh, job.TimeoutLimit)
		}()
