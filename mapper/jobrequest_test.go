...
func TestIncomingJobRequest_ToDomain(t *testing.T) {
	...
	if len(job.SubmissionNodes) != 2 {
		t.Fatalf("expected 2 nodes, got %d", len(job.SubmissionNodes))
	}

	testNode := job.SubmissionNodes[0]
...
func TestConvertToJobRequest_MultipleReplicas(t *testing.T) {
	...
	if len(job.SubmissionNodes) != 3 {
		t.Fatalf("expected 3 nodes (1 test + 2 replicas), got %d", len(job.SubmissionNodes))
	}

	if job.SubmissionNodes[1].Alias != "replica1" || job.SubmissionNodes[2].Alias != "replica2" {
...
func TestConvertToJobRequest_EmptyFiles(t *testing.T) {
	...
	if len(job.SubmissionNodes[0].Files) != 0 || len(job.SubmissionNodes[1].Files) != 0 {
...
func TestConvertToJobRequest_PeersEnvSingleReplica(t *testing.T) {
	...
	// Check replica
	replicaEnvs := make(map[string]string)
	for _, env := range job.SubmissionNodes[1].Envs {
		replicaEnvs[env.Key] = env.Value
	}
	if replicaEnvs["PEERS"] != expectedPeersValue {
